...
BATTERIES_SENSOR_DESCRIPTIONS: tuple[HuaweiSolarSensorEntityDescription, ...] = (
    # ── Battery capacity limits (enabled by default for easy dashboard use) ─
    HuaweiSolarSensorEntityDescription(
        ...
        device_class=SensorDeviceClass.POWER,
        entity_category=EntityCategory.DIAGNOSTIC,
        # Intentionally enabled: useful reference for automation authors and
        # dashboards — was previously hidden behind entity_registry_enabled_default=False.
    ),
    HuaweiSolarSensorEntityDescription(
        ...
        device_class=SensorDeviceClass.POWER,
        entity_category=EntityCategory.DIAGNOSTIC,
        # Intentionally enabled: see note on STORAGE_MAXIMUM_CHARGE_POWER above.
    ),
    # ── End-of-charge SOC (read-only view of the charging cutoff setting) ───
    # This makes the current end-of-charge SOC visible even when parameter
    # configuration is disabled.  The writable counterpart lives in number.py.
    HuaweiSolarSensorEntityDescription(
...
async def async_setup_entry(
    ...
    # Register Modbus telemetry diagnostic sensors for each inverter device
    telemetry_entities = []
    for ucs in device_datas:
        telemetry = ModbusTelemetry.get(ucs.device.serial_number)
        if telemetry:
